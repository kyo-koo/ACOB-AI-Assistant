...
const CodeInterpreter_Copy = () => {
  ...
  const fetchFiles = async () => {
    const resp = await fetch("/api/assistants/codeInterpreter_copy", {
      method: "GET",
    });
  ...
  const handleFileUpload = async (event) => {
    ...
    await fetch("/api/assistants/codeInterpreter_copy", {
      method: "POST",
      body: data,
  ...
  return (
    ...
    <div className={styles.title}>Code Interpreter Copy</div>
    <div
      ...
      >
        {files.length === 0 ? (
          <div className={styles.title}>No file found</div>
        ) : (
...
export default CodeInterpreter_Copy;